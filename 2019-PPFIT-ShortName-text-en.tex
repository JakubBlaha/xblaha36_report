...
The formalism must satisfy the properties listed below. Each property is paired with a concrete use case expressed in abstract placeholders (signals $A$, $B$; events $e_A$, $e_B$; etc.) and a brief note on the construct the formalism provides to address it.
...
\todo{Rest is not reviewed}

\paragraph{Conditional and logical structure.}

\textbf{Value-conditional branches.} The formalism shall support constraints that branch on the current value of an entity. \textit{Example:} ``If $A$ is true then $B$ equals $X$; otherwise $B$ equals $Y$.'' \textit{Solution:} The comparison predicate $\mathit{Cmp}$ and the implication predicate $\mathit{Implies}(A,\, B)$ together express value-guarded branches.

\textbf{Multi-condition guards.} The formalism shall combine conditions of different kinds (event happenings, value comparisons, occurrence counts, historical values) into a single guard. \textit{Example:} ``If $e_A$ occurs and $B > 100$ and $e_C$ has occurred at least twice, then $D$ must follow.'' \textit{Solution:} $\mathit{AllOf}(C)$ takes a finite set of predicates of any kind and forms their conjunction; the formalism's predicates form a uniform space $\mathbf{Pred}$, so event-, value-, count-, and history-based predicates can all be conjoined.

\textbf{Disjunctive triggers.} The formalism shall express a consequence that follows from any one of several alternative triggers. \textit{Example:} ``If $e_A$ or $e_B$ or $e_C$ occurs, then $D$ must hold.'' \textit{Solution:} $\mathit{AnyOf}(C)$ forms the disjunction of a finite set of predicates and may be used as the cause of a $\mathit{Causes}$ construct.

\textbf{Set-valued quantification.} The formalism shall quantify over a finite set of entities and reason about properties of the subset that satisfies a predicate. \textit{Example:} ``For every entity in $\{A, B, C, D\}$, predicate $P$ holds''; or ``at most two entities in $\{A, B, C, D\}$ have a value greater than $10$.'' \textit{Solution:} The Set entity kind together with the quantifiers $\mathit{ForAll}(S,\, P)$ and $\mathit{Exists}(S,\, P)$, plus $\mathit{Filter}$ and $\mathit{Size}$ for cardinality reasoning over the subset that satisfies a predicate.

\paragraph{Temporal patterns.}

\textbf{Sequencing.} The formalism shall require multiple actions to occur in a specified order. \textit{Example:} ``$e_A$ then $e_B$ then $e_C$.'' \textit{Solution:} $\mathit{Sequence}(\psi_1,\, \ldots,\, \psi_n) \Leftrightarrow \exists t_1 \leq t_2 \leq \cdots \leq t_n :\; \psi_i(\rho,\, t_i) \text{ for all } i$.

\textbf{Time-bounded causation.} The formalism shall require an effect to occur within a bounded duration of a cause. \textit{Example:} ``If $e_A$ occurs, $e_B$ must occur within $2$ seconds.'' \textit{Solution:} $\mathit{CausesWithin}(\psi_{\mathit{cond}},\, \psi_{\mathit{effect}},\, d) \Leftrightarrow \forall t :\; \psi_{\mathit{cond}}(\rho,\, t) \Rightarrow \exists t' \in [t,\, t+d] :\; \psi_{\mathit{effect}}(\rho,\, t')$.

\textbf{Sliding temporal windows.} The formalism shall reason about a window of fixed length ending at the current time. \textit{Example:} ``$e_A$ occurred at least twice in the last $10$ seconds.'' \textit{Solution:} An interval constructor applied to $\mathit{Now}$ produces the window, e.g.\ $\mathit{MkIntervalOO}(\mathit{Now} - d,\, \mathit{Now})$, which can then be passed to $\mathit{EvtOccCount}_\rho$ or used as an argument to a $\mathit{ForAll}/\mathit{Exists}$ over time.

\textbf{Existence and global constraints.} The formalism shall express both ``at some point in the trace'' and ``at every time point in the trace''. \textit{Example:} ``Eventually $A$ equals $5$''; ``$A$ is always non-negative''. \textit{Solution:} The temporal constructs $\mathit{Eventually}(\psi) \Leftrightarrow \exists t :\; \psi(\rho,\, t)$ and $\mathit{Always}(\psi) \Leftrightarrow \forall t :\; \psi(\rho,\, t)$.
